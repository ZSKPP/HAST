...
detectors = [PROPOSED_DETECTOR, "OCDD", FEATURE_KS_DETECTOR, "ADWIN", "DDM", "EDDM"]
...
def plot_scenario_wins_three_panels(filename):
    ...
    - the detector order is HAST, OCDD, Feature-KS, ADWIN, DDM, EDDM.
    """
    panel_specs = [
        ("abrupt", "Abrupt drift"),
    ...
    for panel_idx, (ax, (drift_type, panel_title)) in enumerate(zip(axes, panel_specs)):
        ...
        desired_order = [PROPOSED_DETECTOR, "OCDD", FEATURE_KS_DETECTOR, "ADWIN", "DDM", "EDDM"]
    ...
    fig.savefig(OUT_DIR / f"{filename}.png", dpi=300, bbox_inches="tight", pad_inches=0.20)
    fig.savefig(OUT_DIR / f"{filename}.svg", bbox_inches="tight", pad_inches=0.20)
    fig.savefig(OUT_DIR / f"{filename}.pdf", bbox_inches="tight", pad_inches=0.20)
    plt.close(fig)
