...
CONFIG = {
    ...
    # Parameter: FEATURE-KS
    # Feature-wise two-sample Kolmogorov-Smirnov detector.
    # Drift is declared when the fraction of significantly changed
    # features exceeds feature_ks_theta.
...
class FeatureKS:
    ...
    def __init__(
        ...
        self.max_reference_chunks = int(max_reference_chunks)
        self.max_reference_size = self.max_reference_chunks * self.chunk_size
...
def make_feature_ks(config, n_features, chunk_size):
    return FeatureKS(
...
def plot_error_profile_only(repl_results, save_path):
    ...
    ax.set_xlabel('Chunk index', fontsize=15, fontweight='bold')
...
def plot_two_panel_report(repl_results, save_path):
    ...
    detectors_to_plot = ['HAST', 'Feature-KS', 'ADWIN', 'EDDM', 'DDM', 'OCDD']
    detector_labels = {
        'HAST': 'HAST',
    ...
    # BOTTOM PANEL
    # ==========================================
    # BOTTOM PANEL: detector alarm frequency
    # ==========================================
    ax = axes[1]

    n_replications = len(repl_results)

    for det in detectors_to_plot:
        counts = detection_counts[det]

        # Frequency of alarms in each chunk across replications.
        # Example: frequency = 0.7 means that the detector raised
        # an alarm in this chunk in 7 out of 10 replications.
        alarm_frequency = counts / n_replications

        mask = alarm_frequency > 0

        if det == 'HAST':
            ax.scatter(
                x[mask],
                alarm_frequency[mask],
    ...
    ax.set_xlabel('Chunk index', fontsize=15, fontweight='bold')
    ax.set_ylabel('Alarm frequency across replications', fontsize=15, fontweight='bold')
...
def run_replication(seed, n_features, n_drifts, n_chunks, chunk_size, drift_type, gradual_spacing, config):
    ...
    detector_feature_ks = make_feature_ks(config, n_features, chunk_size)
    ...
    for chunk_idx in range(n_chunks):
        ...
        if detector_feature_ks.detect(X_chunk):
            feature_ks_detections.append(chunk_idx)
    ...
    return {
        ...
        'detections': {
            'HAST': padd_detections,
            'Feature-KS': feature_ks_detections,
        ...
        'metrics': {
            'D1': {
                'HAST': calculate_D1(actual_drifts, padd_detections),
                'Feature-KS': calculate_D1(actual_drifts, feature_ks_detections),
            ...
            'D2': {
                'HAST': calculate_D2(actual_drifts, padd_detections),
                'Feature-KS': calculate_D2(actual_drifts, feature_ks_detections),
            ...
            'R': {
                'HAST': calculate_R(n_drifts, len(padd_detections)),
                'Feature-KS': calculate_R(n_drifts, len(feature_ks_detections)),
...
def save_summary_tables(all_results, drift_type, output_dir):
    """
    Saves mean and standard deviation of D1, D2 and R for all scenarios.
    This file can be used later as the source for heatmaps.
    """
    ...
    for scenario_name, result in all_results.items():
        parts = scenario_name.replace("F", "").split("|")
        n_drifts = int(parts[0].strip().split()[0])
        n_features = int(parts[1].strip())

        for metric_name in ["D1", "D2", "R"]:
            metric_values = result["metrics"][metric_name]

            for method_name in method_names:
                values = np.asarray(metric_values[method_name], dtype=float)
                rows.append({
    ...
    try:
        ...
        summary_path = os.path.join(output_dir, f"synthetic_summary_{drift_type}.csv")
        summary_df.to_csv(summary_path, index=False, encoding="utf-8-sig")

        # Wide table: one row per scenario and metric, one mean column per method.
        wide_df = summary_df.pivot_table(
            index=["DriftType", "Metric", "Drifts", "Features", "Scenario"],
            columns="Detector",
            values="Mean",
            aggfunc="first",
        ).reset_index()
        wide_path = os.path.join(output_dir, f"synthetic_summary_{drift_type}_wide.csv")
        ...
        print("Could not save summary CSV files:", exc)
